...
exports.register = async (data) => {
  ...
  if (existing) {
    logger.warn("AuthService.register — email already registered", { email });
    throw new AppError("Email already registered", 409);
  }
...
exports.login = async (email, password) => {
  ...
  logger.info("AuthService.login — started", { email: normalizedEmail });

  const user = await User.findOne({ email: normalizedEmail });
  if (!user) {
    logger.warn("AuthService.login — user not found", { email: normalizedEmail });
    throw new AppError("Invalid credentials", 401);
  }

  const valid = await user.comparePassword(password);
  if (!valid) {
    logger.warn("AuthService.login — wrong password", { email: normalizedEmail });
    throw new AppError("Invalid credentials", 401);
  }
...
exports.forgotPassword = async (email) => {
  const normalizedEmail = normalizeEmail(email);
  logger.info("AuthService.forgotPassword — started", { email: normalizedEmail });

  const user = await User.findOne({ email: normalizedEmail });
  if (!user) {
    logger.warn("AuthService.forgotPassword — user not found", { email: normalizedEmail });
    throw new AppError("No account found with that email", 404);
  }
  ...
  logger.info("AuthService.forgotPassword — token generated", { email: normalizedEmail });
  return { token: resetToken };
};
